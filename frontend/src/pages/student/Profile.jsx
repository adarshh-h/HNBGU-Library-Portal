...
export default function StudentProfile() {
  ...
  return (
    <div className="animate-in page-container page-container--form">
      ...
      <div className="flex gap-1 mb-6 p-1 rounded-lg w-fit" style={{background:"var(--bg-surface-2)",border:"1px solid var(--border)"}}>
        {[['profile', User, 'Profile'], ['password', Lock, 'Password']].map(([id, Icon, label]) => (
          <button key={id} onClick={() => setTab(id)}
            className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-all ${tab === id ? 'shadow-sm' : ''}`}
            style={tab === id ? {background:'var(--bg-surface-3)',color:'var(--text-primary)'} : {color:'var(--text-muted)'}}>
        ...
        <div className="form-section">
          <div className="flex items-center gap-4 pb-4 border-b" style={{borderColor:"var(--border)"}}>
            ...
            <div>
              ...
              <p className="text-sm" style={{color:"var(--text-muted)"}}>{profile?.rollNumber} · {profile?.department}</p>
          ...
          <form onSubmit={handleProfile} className="space-y-4 pt-2">
            ...
            <div className="form-grid">
              <FormField label="Batch">
                <input className="input" style={{background:"var(--input-bg-ro)",color:"var(--input-text-ro)"}} value={profile?.batch || '—'} readOnly />
              </FormField>
              <FormField label="Roll number">
                <input className="input font-mono" style={{background:"var(--input-bg-ro)",color:"var(--input-text-ro)"}} value={profile?.rollNumber || '—'} readOnly />
              </FormField>
            </div>
