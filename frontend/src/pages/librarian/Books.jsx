            ...
            mobile={
              <DataCards>
                {books.map(b => (
                  ...
                  >
                    <DataCardGrid>
                      <DataField label="Accession" value={<span className="font-mono text-xs px-2 py-0.5 rounded inline-block" style={{background:"var(--bg-surface-3)",color:"var(--text-primary)",border:"1px solid var(--border)"}}>{b.accessionNumber}</span>} />
            ...
            desktop={
              <div className="table-scroll">
                <table className="w-full">
                  ...
                  <tbody>
                    {books.map(b => (
                      <tr key={b._id} className="transition-colors hover:bg-surface-hover">
                        <td className="td"><span className="font-mono text-xs px-2 py-0.5 rounded" style={{background:"var(--bg-surface-3)",color:"var(--text-primary)",border:"1px solid var(--border)"}}>{b.accessionNumber}</span></td>
                        <td className="td font-medium max-w-[180px] truncate" style={{color:"var(--text-primary)"}}>{b.bookName}</td>
                        <td className="td" style={{color:"var(--text-muted)"}}>{b.authorName}</td>
      ...
      <Modal open={bulkOpen} onClose={() => { setBulkOpen(false); setBulkResult(null); setCsvFile(null) }} title="Bulk import books">
        {!bulkResult ? (
          <form onSubmit={handleBulk} className="space-y-4">
            <p className="text-sm text-neutral-500 break-words">CSV columns: <span className="font-mono text-xs px-1 rounded break-all" style={{background:"var(--bg-surface-3)",color:"var(--text-primary)"}}>Accession Number, Book Name, Author Name, Category, Publication, Year, Total Pages, Supplier, Price</span></p>
