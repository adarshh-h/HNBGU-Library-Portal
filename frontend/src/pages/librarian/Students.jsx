...
// Defined OUTSIDE Students so its identity is stable across re-renders.
// If it were defined inside, every keystroke would recreate the component
// reference, causing React to unmount + remount it and lose input focus.
function StudentForm({ form, formErr, saving, onChange, onCancel, onSubmit, submitLabel }) {
  ...
  return (
    <form onSubmit={onSubmit} className="space-y-4">
      ...
      <div className="modal-footer !px-0 !pb-0 pt-2">
        <button type="button" onClick={onCancel} className="btn-secondary justify-center w-full">Cancel</button>
...
export default function Students() {
  ...
  const handleFieldChange = (k, v) => setForm(p => ({ ...p, [k]: v }))
            ...
            mobile={
              <DataCards>
                {students.map(s => (
                  ...
                  >
                    <DataCardGrid>
                      <DataField label="Roll No" value={<span className="font-mono text-xs px-2 py-0.5 rounded inline-block" style={{background:"var(--bg-surface-3)",color:"var(--text-primary)",border:"1px solid var(--border)"}}>{s.rollNumber}</span>} />
            ...
            desktop={
              <div className="table-scroll">
                <table className="w-full">
                  ...
                  <tbody>
                    {students.map(s => (
                      <tr key={s._id} className="transition-colors hover:bg-surface-hover">
                        <td className="td font-medium" style={{color:"var(--text-primary)"}}>{s.name}</td>
                        <td className="td"><span className="font-mono text-xs px-2 py-0.5 rounded" style={{background:"var(--bg-surface-3)",color:"var(--text-primary)",border:"1px solid var(--border)"}}>{s.rollNumber}</span></td>
                        <td className="td">{s.department}</td>
                        <td className="td">{s.batch}</td>
                        <td className="td" style={{color:"var(--text-muted)"}}>{s.email}</td>
                        <td className="td">
      ...
      <Modal open={createOpen} onClose={() => { setCreateOpen(false); setNewPassword('') }} title="Add new student" size="lg">
        {newPassword ? (
          <div className="text-center py-4">
            ...
            <p className="font-semibold mb-2" style={{color:"var(--text-primary)"}}>Student created!</p>
            <p className="text-sm text-neutral-500 mb-4">Share this temporary password with the student:</p>
            <div className="bg-neutral-100 rounded-lg px-4 py-3 font-mono text-lg sm:text-xl font-semibold mb-6 break-all" style={{color:"var(--text-primary)"}}>{newPassword}</div>
      ...
      <Modal open={bulkOpen} onClose={() => { setBulkOpen(false); setBulkResult(null); setCsvFile(null) }} title="Bulk import students">
        {!bulkResult ? (
          <form onSubmit={handleBulk} className="space-y-4">
            <p className="text-sm text-neutral-500 break-words">CSV columns required: <span className="font-mono text-xs px-1 rounded break-all" style={{background:"var(--bg-surface-3)",color:"var(--text-primary)"}}>name, email, phone, department, batch, rollNumber</span></p>
