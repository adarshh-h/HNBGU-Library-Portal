...
export default function LibrarianDashboard() {
  ...
  const cards = [
    ...
    { label: 'Books / Student limit', value: '10',     icon: BookMarked,  color: 'text-amber-700',   bg: 'bg-amber-50'   },
  ...
  return (
    <div className="animate-in page-container">
      ...
      {/* Quick actions */}
      <div className="mb-8">
        <h2 className="font-serif font-semibold text-lg mb-4" style={{color:"var(--text-primary)"}}>Quick actions</h2>
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
          {quickLinks.map(({ to, label, desc }) => (
            <Link key={to} to={to}
              className="card p-4 hover:shadow-card-hover transition-all duration-200 group">
              <p className="font-medium text-sm transition-colors group-hover:text-brand-400" style={{color:"var(--text-primary)"}}>{label}</p>
