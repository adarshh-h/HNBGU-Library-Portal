    ...
    try {
        ...
        fs.createReadStream(req.file.path)
            ...
            .on("data", (row) => {
                ...
                // Validate roll number (alphanumeric, e.g., CS2101)
                const rollNumberRegex = /^[A-Za-z0-9]+$/;
                if (!rollNumberRegex.test(row.rollNumber)) {
                    errors.push({ row, error: "Invalid roll number (only letters and numbers allowed)" });
                    return;
                }
            ...
            .on("end", async () => {
                ...
                if (validStudents.length > 0) {
                    try {
                        ...
                        // Handle duplicate key errors thrown by MongoDB (e.g., race conditions)
                        if (insertErr.code === 11000 && insertErr.keyPattern && insertErr.keyPattern.rollNumber) {
                            duplicates.push({ student: insertErr.keyValue, error: "Duplicate roll number (DB constraint)" });
                        }
                    }
                }
